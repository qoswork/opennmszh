...
<h3>编辑监视分类 ${model.node.label}</h3>

<p>
节点 <a href="<c:url value='element/node.jsp?node=${model.node.id}'/>">${model.node.label}</a> (节点ID: ${model.node.id}) 有 ${fn:length(model.node.categories)}个分类 
</p>
...
<form action="admin/categories.htm" method="get">
  ...
  <table class="normal">
    ...
    <tr>
      ...
      <label for="toggleCheckbox">选中，使能控制(请参考警告说明)</label>
      </td>
    </tr>
  </table>
</form>
