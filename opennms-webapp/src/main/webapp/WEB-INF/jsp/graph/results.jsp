...
<c:import url="/includes/header.jsp">
    <c:param name="title" value="资源图结果" />
    <c:param name="headTitle" value="结果" />
    <c:param name="headTitle" value="资源图" />
    <c:param name="headTitle" value="报表" />
    <c:param name="breadcrumb" value="<a href='report/index.jsp'>报表</a>" />
    <c:param name="breadcrumb" value="<a href='graph/index.jsp'>资源图</a>"/>
    <c:param name="breadcrumb" value="结果" />
...
<div id="graph-results">
    ...
    <div id="customTimeForm" name="customTimeForm" ${showCustom}>
        ...
        <form id="range_form" action="${requestScope.relativeRequestPath}" method="get">
            ...
            <input type="submit" value="应用自定义时间"/>
        </form>
    </div>

    <p>
        <strong>从</strong> ${results.start} <br/>
        <strong>到</strong> ${results.end} <br/>
    </p>
        ...
        <c:choose>
            ...
            <c:when test="${!empty resultSet.graphs}"> 
                <c:forEach var="graph" items="${resultSet.graphs}">
                    ...
                    <c:if test="${fn:contains(resultSet.resource.resourceType.label, 'SNMP') || fn:contains(resultSet.resource.resourceType.label, 'TCA') }">
                        <c:if test="${fn:contains(resultSet.resource.label,'(*)') != true}">
                            <a href="javascript:nrtgPopUp('${resultSet.resource.id}','${graph.name}')"><font size="-1"> 开始NRT绘图 ${graph.title} </font></a><br>
                            </c:if>
                        </c:if>
                    ...
                    -->
                    <a href="${zoomUrl}"><img src="${graphUrl}" alt="资源图: ${graph.title} (点击放大)" /></a>
