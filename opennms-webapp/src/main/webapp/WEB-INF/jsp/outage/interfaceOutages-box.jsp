...
<h3><a href="<c:out value="${outageLink}"/>">最近故障</a></h3>

<table>

<% if(outages.length == 0) { %>
  <tr>
    <td>在过去24小时，此接口上没有故障。</td>
  </tr>
<% } else { %>
  <tr>
    <th>服务</th>
  ...
  %>
     ...
     <% } %>
      ...
      <% } else { %>        
        <td class="divider"><fmt:formatDate value="${outage.regainedServiceTime}" type="date" dateStyle="default"/>&nbsp;<fmt:formatDate value="${outage.regainedServiceTime}" type="time" pattern="HH:mm:ss"/></td>
      <% } %>
