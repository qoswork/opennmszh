...
<jsp:include page="/includes/header.jsp" flush="false" >
  <jsp:param name="title" value="通知列表" />
  <jsp:param name="headTitle" value="浏览" />
  <jsp:param name="headTitle" value="通知" />
  <jsp:param name="breadcrumb" value="<a href='notification/index.jsp' title='通知系统页面'>通知</a>" />
...
<% if( parms.ackType == AcknowledgeType.UNACKNOWLEDGED ) { %>
  <p>
    当前只显示 <strong>活动</strong> 通知。
    <a href="<%=this.makeLink(parms, AcknowledgeType.ACKNOWLEDGED)%>" title="显示已确认通知">[显示已确认]</a>
  </p>
<% } else if( parms.ackType == AcknowledgeType.ACKNOWLEDGED ) { %>
  <p>
    当前只显示 <strong>已确认</strong> 通知。  
    <a href="<%=this.makeLink(parms, AcknowledgeType.UNACKNOWLEDGED)%>" title="显示活动通知">[显示活动]</a>
  </p>
<% } %>
    ...
    应用过滤条件:
      <% for( int i = 0; i < length; i++ ) { %>
		<span class="filter"><% Filter filter = (Filter)parms.filters.get(i); %>
				<%=WebSecurityUtils.sanitizeString(filter.getTextDescription())%> <a href="<%=this.makeLink( parms, filter, false)%>" title="删除过滤条件">[-]</a></span> &nbsp; 
      <% } %>
    &mdash; <a href="<%=this.makeLink( parms, new ArrayList<Filter>())%>" title="删除所有过滤条件">[删除所有]</a>
  </p>
<% } %>
	<jsp:include page="/includes/key.jsp" flush="false" />
      ...
      <table>
          ...
          <th class="noWrap">事件ID</th>
          <th>级别</th>
          <th><%=this.makeSortLink( parms, SortStyle.PAGETIME,    SortStyle.REVERSE_PAGETIME,    "pagetime",    "发送时间"    )%></th>
          <th><%=this.makeSortLink( parms, SortStyle.RESPONDER,   SortStyle.REVERSE_RESPONDER,   "answeredby",  "响应"    )%></th>
          <th><%=this.makeSortLink( parms, SortStyle.RESPONDTIME, SortStyle.REVERSE_RESPONDTIME, "respondtime", "响应时间" )%></th>  
          ...
          <% if (responder != null) { %>
            <% Filter responderFilter = new ResponderFilter(responder); %>      
              ...
              <% if( !parms.filters.contains( responderFilter )) { %>
                <a href="<%=this.makeLink( parms, responderFilter, true)%>" class="filterLink" title="只显示此响应的通知">${addPositiveFilter}</a>
              <% } %>
            <% } %>
            </td>
          <td class="divider">
          ...
          <td class="divider">
            <% if(notification.getNodeId() != 0 ) { %>
              ...
              <% if( !parms.filters.contains(nodeFilter) ) { %>
                ...
                <a href="<%=this.makeLink( parms, nodeFilter, true)%>" class="filterLink" title="只显示此节点上的通知">${addPositiveFilter}</a>
                <a href="<%=this.makeLink( parms, new NegativeNodeFilter(notification.getNodeId(), getServletContext()), true)%>" class="filterLink" title="不显示此节点上的通知">${addNegativeFilter}</a>
              <% } %>
            <% } %>
          </td>
          <td class="divider">
            <% if(notification.getIpAddress() != null ) { %>
              ...
              <% if( notification.getNodeId() != 0 ) { %>
                ...
                <a href="<c:out value="${interfaceLink}"/>" title="这个接口上的更多信息"><%=notification.getIpAddress()%></a>
              <% } else { %>
                 <%=notification.getInterfaceId()%>
              <% } %>
              <% if( !parms.filters.contains(intfFilter) ) { %>
                <a href="<%=this.makeLink( parms, intfFilter, true)%>" class="filterLink" title="只显示此IP地址上的通知">${addPositiveFilter}</a>
              <% } %>
            <% } %>
          </td>
          <td class="divider">
            <% if(notification.getServiceName() != null && !notification.getServiceName().trim().isEmpty()) { %>
              ...
              <% if( notification.getNodeId() != 0 && notification.getIpAddress() != null ) { %>
                ...
                <a href="<c:out value="${serviceLink}"/>" title="这个服务上的更多信息"><c:out value="<%=notification.getServiceName()%>"/></a>
              <% } else { %>
                <c:out value="<%=notification.getServiceName()%>"/>
              <% } %>
              <% if( !parms.filters.contains( serviceFilter )) { %>
                <a href="<%=this.makeLink( parms, serviceFilter, true)%>" class="filterLink" title="只显示此服务类型的通知">${addPositiveFilter}</a>
              <% } %>
            <% } %>
          </td>
        </tr>
      ...
      <p><%=notices.length%> 条通知 &nbsp;
        ...
        <% if( parms.ackType == AcknowledgeType.UNACKNOWLEDGED && (request.isUserInRole( Authentication.ROLE_ADMIN ) || !request.isUserInRole( Authentication.ROLE_READONLY ))) { %>
            ...
            <input type="button" value="确认通知" onClick="submitAcknowledge()"/>
        <% } %>
