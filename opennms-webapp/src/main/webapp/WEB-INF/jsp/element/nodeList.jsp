...
<jsp:include page="/includes/header.jsp" flush="false" >
  <jsp:param name="title" value="节点列表" />
  <jsp:param name="headTitle" value="节点列表" />
  <jsp:param name="location" value="nodelist" />
  <jsp:param name="breadcrumb" value="<a href ='element/index.jsp'>查询</a>"/>
  <jsp:param name="breadcrumb" value="节点列表"/>
</jsp:include>
...
<p>
  <c:choose>
    <c:when test="${model.nodeCount == 1}">
      <c:set var="nodePluralized" value="节点"/>
    </c:when>
  ...
  <c:choose>
    <c:when test="${model.interfaceCount == 1}">
      <c:set var="interfacePluralized" value="接口"/>
    </c:when>
  ...
  <c:choose>
    <c:when test="${!command.listInterfaces}">
    <a href="${thisURL}">显示接口</a>
    </c:when>
    <c:otherwise>
    <a href="${thisURL}">隐藏接口</a>
    </c:otherwise>
  </c:choose>
</p>
