...
<%
String nodeBreadCrumb = "<a href='element/node.jsp?node=" + nodeId  + "'>节点</a>";
%>
<jsp:include page="/includes/header.jsp" flush="false" >
  <jsp:param name="title" value="接口" />
  <jsp:param name="headTitle" value="<%= ipAddr %>" />
  <jsp:param name="headTitle" value="接口" />
  <jsp:param name="breadcrumb" value="<a href='element/index.jsp'>查询</a>" />
  <jsp:param name="breadcrumb" value="<%= nodeBreadCrumb %>" />
  <jsp:param name="breadcrumb" value="接口" />
</jsp:include>
...
function doDelete() {
     if (confirm("你确定要继续吗？此操作将永久删除这个接口？并不能恢复。"))
     {
...
%>
      ...
      <ul>
        <% if (! ipAddr.equals("0.0.0.0")) { %>
          <li>
            <a href="<c:out value="${eventUrl1}"/>">查看IP地址的事件</a>
          </li>
        <% } %>

        <% if (ifIndex > 0 ) { %>
          <li>
            <a href="<c:out value="${eventUrl2}"/>">查看接口索引的事件</a>
          </li>
        <% } %>
          ...
          for (OnmsResource resource : resources) {
              if (resource.getName().equals(ipAddr) || resource.getName().equals(ifLabel)) {
                  %>
                      ...
                      <li>
                          <a href="<c:out value="${graphLink}"/>"><c:out value="<%=resource.getResourceType().getLabel()%>"/>图</a>
                      </li>
                  <% 
              }
          }
        ...
        <% if (request.isUserInRole( Authentication.ROLE_ADMIN )) { %>
          <li>
            <a href="admin/deleteInterface" onClick="return doDelete()">删除</a>
          </li>
        <% } %>
        ...
        <% if (request.isUserInRole( Authentication.ROLE_ADMIN )) { %>
          <li>
            ...
            <a href="<c:out value="${rescanUrl}"/>">重新扫描</a>
          </li>
        <% } %>
        ...
        <% if (request.isUserInRole( Authentication.ROLE_ADMIN )) { %>
          <li>
            ...
            <a href="<c:out value="${schedOutageUrl}"/>">计划故障</a>      
          </li>
        <% } %>
...
<div id="contentright">
            ...
            <% if (!ipAddr.equals("0.0.0.0")) { %>
              <c:set var="eventHeader1">
                <a href="<c:out value="${eventUrl1}"/>">最近发生的事件 (使用IP地址过滤: <c:out value="<%=ipAddr%>"/>)</a>
              </c:set>
            ...
            <% if (ifIndex > 0 ) { %>
              <c:set var="eventHeader2">
                <a href="<c:out value="${eventUrl2}"/>">最近发生的事件 (使用接口索引过滤: <c:out value="<%=ifIndex%>"/>)</a>
              </c:set>
              <jsp:include page="/includes/eventlist.jsp" flush="false" >
                <jsp:param name="node" value="<%=nodeId%>" />
                <jsp:param name="throttle" value="5" />
