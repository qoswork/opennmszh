...
<tbody>
<tr>
<td style="background: #999 url(images/keyGradient.png) repeat-x" title="${param.clearedCaption == null ? '已清除' : param.clearedCaption}">	</td>
<td style="background: #336600 url(images/keyGradient.png) repeat-x" title="${param.normalCaption == null ? '一般:信息性消息。不需要采取任何行动。' : param.normalCaption}">	</td>
<td style="background: #999000 url(images/keyGradient.png) repeat-x" title="${param.indetermCaption == null ? '不确定:事件不能关联到任何级别。' : param.indetermCaption}">	</td>
<td style="background: #FFCC00 url(images/keyGradient.png) repeat-x" title="${param.warnCaption == null ? '警告:可能需要采取行动。若可以应该被记录下来。' : param.warnCaption}">	</td>
<td style="background: #FF9900 url(images/keyGradient.png) repeat-x" title="${param.minorCaption == null ? '次要:一个设备的一部分(服务，接口，电源等)。需要引起注意。' : param.minorCaption}">	</td>
<td style="background: #FF3300 url(images/keyGradient.png) repeat-x" title="${param.majorCaption == null ? '主要:设备完全Down或处于危险之中。需要立即引起注意。' : param.majorCaption}">	</td>
<td style="background: #CC0000 url(images/keyGradient.png) repeat-x" title="${param.criticalCaption == null ? '严重:众多设备受到影响，解决这个问题是至关重要的。' : param.criticalCaption}">	</td>
</tr>
</tbody>
</table>
</td>
