...
<p class="pager">
 <% if (limit > 0 ) { %> 
  结果:(<%=startResult%>-<%=endResult%> of <%=count%>)
 <% } else { %>
  All Results
 <% } %> 
  ...
  <% if( count > limit ) { %>  
    <span>
<% if( multiple > 0 ) { %>
      <a href="<%=baseUrl%>&amp;<%=multipleName%>=0&amp;<%=limitName%>=<%=limit%>">第一页</a>&nbsp;  
      <a href="<%=baseUrl%>&amp;<%=multipleName%>=<%=multiple-1%>&amp;<%=limitName%>=<%=limit%>">上一页</a>&nbsp;  
    <% } %>
    ...
    <% if( multiple < highestPossibleIndex ) { %>
      <a href="<%=baseUrl%>&amp;<%=multipleName%>=<%=multiple+1%>&amp;<%=limitName%>=<%=limit%>">下一页</a>&nbsp;
      <a href="<%=baseUrl%>&amp;<%=multipleName%>=<%=highestPossibleIndex%>&amp;<%=limitName%>=<%=limit%>">最后一页</a>
    <% } %>
		</span>
   <% } %>      
</p>
