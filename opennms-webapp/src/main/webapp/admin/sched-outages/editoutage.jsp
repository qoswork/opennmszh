...
<body>
<p>
不能找到要编辑的故障，根据指定的名称参数在存储的会话里。不能编辑！
</p>
</body>
</html>
<%
...
<jsp:include page="/includes/header.jsp" flush="false">
	<jsp:param name="title" value="编辑故障" />
	<jsp:param name="headTitle" value="编辑" />
	<jsp:param name="headTitle" value="计划故障" />
	<jsp:param name="headTitle" value="管理" />
	<jsp:param name="location" value="admin" />
	<jsp:param name="breadcrumb"
		value="<a href='admin/index.jsp'>管理</a>" />
	<jsp:param name="breadcrumb"
		value="<a href='admin/sched-outages/index.jsp'>计划故障</a>" />
	<jsp:param name="breadcrumb" value="编辑" />
</jsp:include>
...
<h2>编辑故障: <%= theOutage.getName() %></h2>

		<label>节点和接口:</label>
			<table class="normal" border="0">
				<tr>
					<th valign="top">节点名称</th>
					<th valign="top">接口</th>
				</tr>
				<tr>
					<td valign="top">
						<form id="addNode" action="admin/sched-outages/editoutage.jsp" method="post">
							...
							<p style="font-weight: bold; margin-bottom: 2px;">查询 (最大200条):</p>
							<div class="ui-widget">
								<select id="newNodeSelect" name="newNodeSelect" style="display: none"></select>
								<input type="radio"  value="addPathOutageDependency" name="addPathOutageNodeRadio"/> 添加路径故障依赖
								<input type="submit" value="添加" name="addNodeButton"/>
							</div>
						</form>
						<p style="font-weight: bold; margin: 10px 0px 2px 0px;">目前的选择:</p>
						<% {
						if (hasMatchAny) {
							%>
							<p><i>所有节点</i></p>
					...
					<td valign="top">
						<form id="addInterface" action="admin/sched-outages/editoutage.jsp" method="post">
							...
							<div class="ui-widget">
								...
								<input type="radio"  value="addPathOutageDependency" name="addPathOutageInterfaceRadio"/> 添加路径故障依赖
								<input type="submit" value="添加" name="addInterfaceButton"/>
							</div>
						</form>
						<p style="font-weight: bold; margin: 10px 0px 2px 0px;">目前的选择:</p>
						<%
								...
								<form id="deleteInterfaces" action="admin/sched-outages/editoutage.jsp" method="post">
									...
									<% for (int i = 0; i < outageInterfaces.length; i++) {
										...
										if (interfaces.length > 0) {
											...
											<i>不能发现 <c:out value="<%=addr%>"/> 在数据库里</i><br/>
										<% }
									} %>
								</form>
							<% } else { %>
				...
				<tr>
					<td>
						...
						<span style="<%= theOutage.getType() == null? "" : "display: none" %>">
							...
							<input type="submit" value="设置" name="setOutageType" />
						</span>
					</td>
				</tr>
			</table>
		<label>时间:</label>
			...
			<table class="normal" border="0">
				...
				<tr>
					<td>
						<input type="submit" value="添加故障" name="addOutage" />
					</td>
				</tr>
				<% if (theOutage.getTimeCount() == 0) { %>
					<tr>
						<td><span style="color: #ff0000">你必须至少定义一个时间跨度。</span></td>
					</tr>
				<% } %>
			</table>
		<label>适用于:</label>
			<ul class="treeview">
				<li>
					<p>通知:</p>
					<ul>
						<li><input type="checkbox" <%=(enabledOutages.contains("notifications"))?"checked=\"checked\"":""%> name="notifications" id="notifications"/> <label for="notifications">所有通知</label> </li>
					</ul>
				</li>
				<li>
					<p>轮询状态:</p>
			...
			%><input type="submit" value="保存" name="saveButton" /><% } %>
			<input type="submit" value="取消" name="cancelButton" />
</form>
