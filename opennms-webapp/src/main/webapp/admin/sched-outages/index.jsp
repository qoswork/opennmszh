...
<table id="outages" border="1" cellpadding="5">
	...
	<tr valign="top">
		...
		%> <%=elementNode == null || elementNode.getType().charAt(0) == 'D' ? "节点: Node ID " + nodeList[j].getId() + " 未找到" : "节点: " + elementNode.getLabel()%><br/>
		<%
			}
				...
				for (int j = 0; j < interfaceList.length; j++) {
					...
					if ("match-any".equals(rawAddress)) {
						display = new StringBuffer("所有节点/接口");
					} else {
						...
						if (nodeids.size() == 0) {
							display.append("接口: " + rawAddress + " 未找到<br/>");
						}
						for (Integer nodeid: nodeids) {
							...
							if (thisInterface.isManagedChar()=='D') {
								display.append("接口: " + thisInterface.getIpAddress() + " 未找到<br/>");
							} else {
								if (thisInterface.getHostname() != null && !thisInterface.getHostname().equals(thisInterface.getIpAddress())) {
									display.append("接口: " + thisInterface.getIpAddress() + " " + thisInterface.getHostname());
								} else {
									display.append("接口: " + thisInterface.getIpAddress());
								}
								if (thisInterface.isManaged()) {
									display.append("<br/>");
								} else {
									display.append(" (未管理)<br/>");
								}
							}
						}
					}
		...
		<td><a id="<%=outageName%>.edit"
			href="admin/sched-outages/editoutage.jsp?name=<%=java.net.URLEncoder.encode(outageName, "UTF-8")%>">编辑</a></td>
		<td><a id="<%=outageName%>.delete"
			href="admin/sched-outages/index.jsp?deleteOutage=<%=java.net.URLEncoder.encode(outageName, "UTF-8")%>"
			onClick="if(!confirm('你确定要删除此故障吗？')) {return false;}">删除</a></td>
	</tr>
...
<form action="admin/sched-outages/editoutage.jsp" method="post" >
  <input type="text" value="新名称" size="40" name="newName" /> 
  <input type="hidden" name="addNew" value="true" /> 
  <input type="submit" name="newOutage" value="添加新故障" />
</form>
